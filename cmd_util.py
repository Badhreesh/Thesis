...
def make_doom_env(doom_lvl, num_env, seed, start_index=0):
    """
    Create a wrapped, monitored SubprocVecEnv for Doom.
    """
    def make_env(rank):
        def _thunk():
            ...
            env = Monitor(env, logger.get_dir() and os.path.join(logger.get_dir(), str(rank))) # Same
            #return wrap_deepmind(env, **wrapper_kwargs) # Needs to go. THis does preprocessing, like stacking frames.
            return env                                           # Not needed for doom Look into wrapper_kwargs, what it does. Return env.
        return _thunk
    set_global_seeds(seed) # Same
...
def doom_arg_parser():
    ...
    parser = arg_parser()
    parser.add_argument('--doom-lvl', type=str, help='Doom Level', choices=gym_tasks, default='Doom-hg_simple-v0')
    parser.add_argument('--seed', help='RNG seed', type=int, default=0)
    parser.add_argument('--num-timesteps', type=int, default=int(5000000))
    return parser    
